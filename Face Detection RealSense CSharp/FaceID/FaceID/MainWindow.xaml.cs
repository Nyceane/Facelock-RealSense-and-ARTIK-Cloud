...
namespace FaceID
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private bool lastFrameDetected = false;
        private bool isDetected = false;

        private static string DEVICEID = "<DEVICE ID>";
        private string OAUTH_TOKEN = "<AUTH TOKEN>";
        ...
        public MainWindow()
        {
            ...
            Configuration.Default.AccessToken = OAUTH_TOKEN;

            // Parse is used for store user photos
            ParseClient.Initialize("<PARSE ID>", "<PARSE TOKEN>");
        ...
        private void ConfigureRealSense()
        {
            ...
            if (senseManager == null || senseManager.captureManager == null)
            {
                return;
            }
                    ...
                    if (numFacesDetected > 0)
                    {
                        ...
                        if (face != null)
                        {
                            ...
                            if (recognitionData.IsRegistered())
                            {
                                ...
                                if (isDetected == true && lastFrameDetected == false && CheckForInternetConnection())
                                {
                                    colorBitmap.Save("image2send.jpg", System.Drawing.Imaging.ImageFormat.Jpeg);
                                    lastFrameDetected = true;
                                    AccessTheWebAsync(userId);
                                }
        ...
        async Task<bool> AccessTheWebAsync(String id)
        {
            var apiInstance = new MessagesApi();
            var message = new Message(); // Message | Message object that is passed in the body
            message.Sdid = DEVICEID;
            message.Data = new System.Collections.Generic.Dictionary<string, object>();
            message.Data.Add("facialdetection", true);
            try
            {
                // Send Message
                MessageIDEnvelope result = apiInstance.SendMessage(message);
                Console.WriteLine(result);
            }
            catch (Exception e)
            {
                Console.WriteLine("Exception when calling MessagesApi.SendMessage: " + e.Message);
            }

            byte[] data = File.ReadAllBytes("image2send.jpg");

            ParseFile file = new ParseFile("visitor_image.jpg", data);
            await file.SaveAsync();

            ParseObject visitor = new ParseObject("Visitors");
            visitor["Photo"] = file;
        ...
        {
            this.Dispatcher.Invoke(System.Windows.Threading.DispatcherPriority.Normal, new System.Action(delegate()
            {
